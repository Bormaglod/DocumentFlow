...
public class WIAScanner : IScanner
{
    ...
    {
        PerformDeviceAction(device => ScannerPropertiesDialog.ShowPropertiesDialog(device.Properties));
    }
    ...
    public void ShowItemListProperties()
    {
        ...
        if (items.Count > 0)
        {
            ScannerPropertiesDialog.ShowPropertiesDialog(items[1].Properties);
        }
        else
        {
            throw new Exception("No items have been selected.");
    ...
    public IList<Image> Scan(int pages, int resolution = 300, WIAPageSize pageSize = WIAPageSize.A4)
    {
