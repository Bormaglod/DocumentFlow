...
namespace DocumentFlow
{
    public partial class ViewerControl : UserControl, ISettings, IBrowser
    {
        ...
        public Guid CurrentId => gridContent.SelectedItem is IIdentifier<Guid> row ? row.id : Guid.Empty;
        ...
        public void InitializeViewer()
        {
            ...
            if (Parent is DockContent content)
            {
                content.TabText = ExecutedCommand.name;
            }
            ...
            switch (dataType)
            {
                ...
                case DataType.Report:
                    ...
                    UserActionComparer commandComparer = new();
...
#pragma warning restore CA1822 // Пометьте члены как статические
        ...
        private void GroupAction(Guid? id, CommandAction action)
        {
            ...
            GroupEditorForm editor = new(ExecutedCommand.EntityKind.code);
...
#if USE_LISTENER
        private static int GetRowIndex(IList list, Guid id)
        {
        ...
        private void DocumenuMenuItem_Click(object sender, EventArgs e)
        {
            if (sender is ToolStripMenuItem menuItem && menuItem.Tag is DocumentRefs doc)
            {
                ...
                DocumentRefEditorForm refEditor = new(ftpPath);
