...
{
    public partial class DiagramDockControl : DockContent, IPage
    {
        private readonly IContainerPage containerPage;
        private readonly Data.Entities.Transition transition;

        public DiagramDockControl(IContainerPage container, Guid transitionId)
        {
            InitializeComponent();
        ...
        public static string Code(Guid id) => $"diagram [{id}]";

        string IPage.Code => Code(transition.id);
        ...
        private static Label CreateLabel(PathNode node, string labelText)
        {
            Label label = new(node, labelText);
        ...
        private static void CreatePorts(FilledPath rect)
        {
            for (var p = Position.TopLeft; p < Position.Custom; p++)
            {
                ConnectionPoint point = new()
                {
                    Position = p
                };
        ...
        {
            RoundRect rect = new(x, y, 30, 16, MeasureUnits.Millimeter);
        ...
        {
            LineConnector line = new(left.PinPoint, right.PinPoint);
        ...
        private void CreateExtremeShape(IDictionary<int, RoundRect> nodes, int? status, int x, int y, bool reverse)
        {
            ...
            if (nodes.ContainsKey(status.Value))
            {
                ...
                Ellipse node = new(x, y, 10, 10, MeasureUnits.Millimeter);
        ...
        private void CreateDiagram()
        {
            ...
            Dictionary<int, RoundRect> nodes = new();
            int x = 40;
            int y = 20;
        ...
        private void buttonPrint_Click(object sender, EventArgs e)
        {
            if (diagram1 != null)
            {
                ...
                System.Windows.Forms.PrintDialog printDlg = new()
                {
                    Document = printDoc,
                    AllowSomePages = true
                };
        ...
        private void buttonPreview_Click(object sender, EventArgs e)
        {
            ...
            {
                PrintDocument printDoc = diagram1.CreatePrintDocument();
                System.Windows.Forms.PrintPreviewDialog printPreviewDlg = new();
